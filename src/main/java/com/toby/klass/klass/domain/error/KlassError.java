...
public enum KlassError implements ErrorCode {
    ...
    /**
     * 취소 가능 기간을 {@code DRAFT} 아닌 상태에서 바꾸려 했다.
     *
     * <p>취소 가능 기간은 <b>수강생과의 약속</b>이다. {@code OPEN} 이 되어 신청자가 생긴 뒤에
     * 바꾸면 이미 신청한 사람의 취소 조건이 사후에 불리하게 바뀔 수 있다. {@code DRAFT} 는
     * 신청 자체가 불가능하므로(ERD 정본 §2.2) 그때까지만 허용한다.
     *
     * <p><b>409 인 근거</b>: 요청 값 자체는 유효하고 <b>현재 리소스 상태와 충돌</b>한다
     * (Design §6.2). 400 은 "입력을 고쳐 다시 보내라"로 읽혀 클라이언트가 값을 바꿔 재시도하게
     * 만드는데, 어떤 값을 보내도 강의가 {@code DRAFT} 로 돌아가지 않는 한 성공하지 않는다.
     *
     * <p><b>{@link #INVALID_KLASS_STATUS_TRANSITION} 을 재사용하지 않는 이유</b>: 그것은
     * <b>상태 전이</b>가 실패했다는 뜻이고 이것은 <b>필드 수정</b>이 실패했다는 뜻이다. 같은
     * 코드를 주면 클라이언트가 상태 변경 API({@code PATCH /v1/klasses/{id}/status})를 다시
     * 호출하려 든다 — 고쳐야 할 요청은 수정 API 쪽인데 엉뚱한 곳을 보게 된다.
     *
     * <p><b>같은 값 재전송은 이 코드에 도달하지 않는다.</b> 수정은 전체 교체라(D-25) 모든
     * 요청이 이 필드를 싣고 오므로, 값이 실제로 달라질 때만 거부한다 (Design D-26).
     */
